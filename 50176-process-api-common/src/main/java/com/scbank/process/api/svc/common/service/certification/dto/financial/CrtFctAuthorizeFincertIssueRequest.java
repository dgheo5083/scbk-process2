package com.scbank.process.api.svc.common.service.certification.dto.financial;

import com.scbank.process.api.fw.message.IMessageObject;
import com.scbank.process.api.fw.message.annotation.IntegrationMessage;
import com.scbank.process.api.fw.message.annotation.IntegrationMessage.Type;
import com.scbank.process.api.fw.message.annotation.MessageField;

import lombok.Data;

/**
 * CSL 서비스 요청 정보 클래스
 * 금융인증서 발급
 */
@Data
@IntegrationMessage(id = "CrtFctAuthorizeFincertIssueRequest", type = Type.REQUEST)
public class CrtFctAuthorizeFincertIssueRequest implements IMessageObject {

	@MessageField(id = "email1", name = "이메일1", defaultValue = "")
	private String email1;

	@MessageField(id = "email2", name = "이메일2", defaultValue = "")
	private String email2;

	@MessageField(id = "safeCardNum", name = "안전카드값")
	private String safeCardNum;

	@MessageField(id = "safeCardNum2", name = "안전카드값2")
	private String safeCardNum2;

	@MessageField(id = "safeCardSeq1", name = "안전카드일련번호 사용자입력값1")
	private String safeCardSeq1;

	@MessageField(id = "safeCardSeq2", name = "안전카드일련번호 사용자입력값2")
	private String safeCardSeq2;

	@MessageField(id = "safeCardSeq3", name = "안전카드일련번호 사용자입력값3")
	private String safeCardSeq3;

	@MessageField(id = "tel1", name = "전화번호")
	private String tel1;

	@MessageField(id = "tel2", name = "전화번호")
	private String tel2;

	@MessageField(id = "tel3", name = "전화번호")
	private String tel3;
	
	@MessageField(id = "refNum", name = "참조번호")
	private String refNum;
	
	@MessageField(id = "appCode", name = "인가코드")
	private String appCode;

}