...
@Getter
@Setter
@IntegrationMessage(id = "CbTbs03D21100Req", type = Type.REQUEST, description = "즉시이체 본거래 요청 전문", captureSystem = "OLTP")
public class CbTbs03D21100Req implements IMessageObject {
    ...
    private String CgAcctPassword; // _DNFE2E_ _/DNFE2E_

    @MessageField(id = "JgCode", name = "지급구분", length = 1, align = AlignType.LEFT, padding = StringUtils.SPACE)
    private String JgCode;

    // @MessageField(id = "HgSNameSO1", name = "비고란(송금인명)입금통장표시내용 한글시작", length = 1,
    // align = AlignType.LEFT)
    // private String HgSNameSO1; // 0x0E

    @MessageField(id = "HgSName", name = "비고란(송금인명)입금통장표시내용", length = 26, align = AlignType.LEFT, sosi = true)
    private String HgSName; // 0x20

    // @MessageField(id = "HgSNameSI1", name = "비고란(송금인명)입금통장표시내용 한글끝", length = 1,
    // align = AlignType.LEFT)
    // private String HgSNameSI1; // 0x0F
    ...
    // @MessageField(id = "HgRNameSO2", name = "출금통장표시내용 한글시작", length = 1, align =
    // AlignType.LEFT)
    // private String HgRNameSO2; // 0x0E

    @MessageField(id = "HgRName", name = "출금통장표시내용", length = 22, align = AlignType.LEFT, sosi = true)
    private String HgRName;

    // @MessageField(id = "HgRNameSI2", name = "출금통장표시내용 한글끝", length = 1)
    // private String HgRNameSI2; // 0x0F
    ...
    @MessageField(id = "YINEWIPGJ", name = "신입금계좌여부", length = 1, align = AlignType.LEFT, padding = StringUtils.SPACE)
    private String YINEWIPGJ;

    @MessageField(id = "YISIMPLE", name = "간편이체여부", length = 1, align = AlignType.LEFT, padding = StringUtils.SPACE)
    private String YISIMPLE;

    @MessageField(id = "YIMOBILE3", name = "모바일3.0여부", length = 1, align = AlignType.LEFT, padding = StringUtils.SPACE)
    private String YIMOBILE3;

    @MessageField(id = "YIPRESENT", name = "선물하기여부 Y", length = 1, align = AlignType.LEFT, padding = StringUtils.SPACE)
