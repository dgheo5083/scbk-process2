...
@Data
@Builder
@IntegrationMessage(id = "CbIbf01H52400Req", type = Type.REQUEST, captureSystem = "OLTP", description = "외화보통예금/당좌예금입금")
public class CbIbf01H52400Req implements IMessageObject {
    ...
    @MessageField(id = "wonAcctPass", name = "원화계좌비밀번호", length = 4, align = AlignType.LEFT, padding = StringUtils.SPACE, masking = true, maskingType = "03")
    private String wonAcctPass;
    ...
    @MessageField(id = "rcvFrCurr", name = "입금외화통화", length = 3, align = AlignType.LEFT, padding = StringUtils.SPACE)
    private String rcvFrCurr;

    @MessageField(id = "rcvFrAmt", name = "입금외화금액", length = 15, align = AlignType.RIGHT, padding = StringUtils.ZERO)
    private BigDecimal rcvFrAmt;

    @MessageField(id = "applyCurr", name = "적용환율", length = 7, align = AlignType.RIGHT, padding = StringUtils.ZERO)
    private Integer applyCurr;

    @MessageField(id = "payWonAmt", name = "출금원화금액", length = 15, align = AlignType.RIGHT, padding = StringUtils.ZERO)
    private BigDecimal payWonAmt;
    ...
    @MessageField(id = "YIORDID", name = "WFX ORDER ID", length = 21, align = AlignType.LEFT, padding = StringUtils.SPACE)
