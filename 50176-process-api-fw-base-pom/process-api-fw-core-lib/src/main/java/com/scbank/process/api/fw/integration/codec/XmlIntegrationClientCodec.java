...
/**
 * XML 전문 포맷을 처리하는 연계 시스템용 Codec 구현체
 *
 * <p>
 * {@link XmlMessageMapper}를 이용하여 요청 객체를 고정 길이 byte[]로 직렬화하거나,
...
public class XmlIntegrationClientCodec implements IntegrationClientCodec {
    ...
    public <T extends IMessageObject> byte[] encode(MessageContext context, T request)
            throws Exception {
        return this.xmlMessageMapper.serialize(request, context);
    }
    ...
    public <T extends IMessageObject> T decode(MessageContext context, byte[] response,
        ...
        return this.xmlMessageMapper.deserialize(response, responseType, context);
    }
}
