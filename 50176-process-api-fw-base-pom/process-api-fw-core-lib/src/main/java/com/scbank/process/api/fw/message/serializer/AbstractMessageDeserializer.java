...
public abstract class AbstractMessageDeserializer {
    ...
    protected int getRepeatCount(IMessageFieldMetadata metadata, IMessageObject object, MessageContext ctx) {
        ...
        switch (repeatType) {
            ...
            case REFERENCE -> {
                ...
                try {
                    value = metadata.getRepeatCount();
                    Object cnt = ctx.getPathValue(value);
                    repeatCount = Integer.parseInt(String.valueOf(cnt));
                } catch (NumberFormatException e) {
     ...
     * @return
     */
    protected boolean checkConditionalField(IMessageFieldMetadata metadata, MessageContext ctx) {
